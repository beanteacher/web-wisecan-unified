...
차단 행위: 실발송, 결제·환불, 세금계산서, 카카오 템플릿 심사 신청, RCS 브랜드 등록.
...
flowchart LR
  ...
  ME_GRP --> BIZUP["/signup/business<br>사업자 전환 신청"]
```

> 좌측 사이드바 그룹 순서 = 사용자 빈도순 (발송 → 이력 → 발신번호 → 키 → 템플릿 → 결제 → 회사 → 지원 → 내 계정).
> 하위 계정(`COMPANY_SUB_ACCOUNT`)·개인 회원(`MEMBER`, `company_id IS NULL`)은 회사 그룹 사이드바가 숨김. `COMPANY_MASTER` 에게만 노출.
...
flowchart LR
  ...
  AHOME --> REV_GRP{심사}
  REV_GRP --> R_BIZ["/admin/review/business<br>사업자 전환"]
...
| PG-PUB-002 | `/signup` 개인 가입 (회사 직원도 동일 절차) | — | `04 §1.2` |
...
| PG-BIZUP-001 | `/signup/business` 사업자 전환 신청 (v2 — 공개→회원 이동) | 회원 | `04 §1.3` |
...
> 페이지 총계 — 공개 9 / 회원 콘솔 24 / 어드민 17 = **50개**. 체험은 회원 콘솔 트리 재사용으로 별도 카운트하지 않음. (PG-PUB-003 사업자 전환은 v2 에서 회원 콘솔 PG-BIZUP-001 로 이동 — PG-PUB-003 결번)
