...
| PG-TRY-001 | `/try` (체험 진입 + 워터마크 배너) | 익명 체험 세션 | RQ-TRY-001~016 |
...
| PG-CO-002 | `/company/master-roles` (마스터 권한 부여) | COMPANY_MASTER | RQ-MEMBER-201~204 |
...
| PG-AD-013 | `/admin/dashboard` (운영 대시보드·통계) | ADMIN | RQ-ADMIN-801~803, RQ-TRY-015 |
...
```
[비회원] / → "체험하기" 클릭
   ↓ 시스템: 익명 체험 세션 발급 (RQ-TRY-002)
   ↓ 더미 회원사 컨텍스트 + 더미 발신번호·키·발송 이력·잔액 사전 적재 (RQ-TRY-003)
[비회원/체험 세션] /try → 회원과 동일 UI 노출 (RQ-TRY-004)
   ↓ 상시 워터마크 "체험 모드" (RQ-TRY-009)
   ↓ 발송 시도 → 실송출 차단, 가상 결과 즉시 반환 (RQ-TRY-005)
   ↓ 결제 시도 → 차단, 시뮬레이션 결과만 (RQ-TRY-006)
   ↓ "회원가입하고 실제로 사용하기" CTA 노출 (RQ-TRY-010)
[비회원]
   ↓ 가입 완료 → 더미 데이터는 폐기, 운영 계정 새로 시작 (RQ-TRY-014)
```
...
### 4.2. 회사 마스터 모델 — DB 스키마 (1차 도입 확정 ✅)

> §A 결정 (2026-04-30) — 권한 부여형으로 1차 구현. 미도입 트리 종속형 스키마는 1차 범위 외 (§14.A 결정 마커 참조).
...
wsc callback list                                           # RQ-CLI-204
wsc callback register --number 0212345678 --type personal   # RQ-CLI-205
wsc callback delete clr_01HQABC                             # RQ-CLI-206
...
| 발신번호 목록 | `wsc.callback.list` (RQ-MCP-008) | `wsc callback list` (RQ-CLI-204) | ✓ |
...
| **보안 민감 액션** (키 발급/폐기·발신번호 등록/삭제·결제·충전·환불·스코프 변경) | **노출 X** (RQ-MCP-014) | `wsc callback register/delete`, `wsc key create/revoke` 등 명시 명령 (RQ-CLI-205·206·208·209) | 비대칭 (의도) |
...
### 13.1. §A 회사 마스터 권한 — 1차 도입 확정 ✅

| 항목 | 1차 도입 구현 (권한 부여형) ✅ |
|---|---|
| **DB** | `accounts` + `account_role` 다대다 (§4.2 스키마 참조) |
| **발신번호 종속** | `callback_ids.company_id` |
| **권한 이관** | `account_role` 행 갱신 |
| **법인 대표번호** | 권한 보유자만 사용 |
| **세분 권한 추가** | 유연 (역할 추가) |
| **감사 추적** | `account_role.granted_by` |
