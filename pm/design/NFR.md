...
- PRD §5.3 (운영 품질/SLA)와 현 `pm/design/FS.md` §12 (비기능 요약 표), `pm/design/SRS.md` 의 RQ-ARCH·RQ-SEC·RQ-TRY 중 비기능 측면을 통합·승격해 본 문서로 분리한다.
...
| NFR-AVAIL-006 | 체험 모드 (`/try`) | 99.0% | ≤ 7시간 18분 | RQ-TRY-008 (운영 DB와 분리) |
...
| NFR-SEC-307 | 체험 모드 어뷰징 차단 | 동일 IP/디바이스 단기 다수 발급 차단 (RQ-TRY-016) |
...
| NFR-PRIV-206 | 체험 모드 데이터 | 세션 만료 즉시 폐기 | RQ-TRY-012·014 | RQ-TRY-008 |
...
| NFR-DATA-107 | 체험 세션 데이터 | 세션 만료 즉시 폐기 (RQ-TRY-012) |
...
| NFR-ID | 항목 | 정책 | 출처 |
|---|---|---|---|
| NFR-DATA-301 | 체험 모드 격리 | 운영 DB와 물리/논리 분리, 체험 행위가 운영 데이터에 영향 없음 | RQ-TRY-008 |
...
| NFR-ID | 항목 | 정책 | 출처 |
|---|---|---|---|
| NFR-USE-401 | 체험 모드 진입 시간 | 클릭 후 ≤ 2초 (더미 데이터 사전 적재) | RQ-TRY-001 |
| NFR-USE-402 | 체험 모드 워터마크 | 모든 페이지 상시 노출 | RQ-TRY-009 |
| NFR-USE-403 | 가입 전환 CTA | 발송 직후·결제 시도·이력 조회 시 노출 | RQ-TRY-010 |
| NFR-USE-404 | 온보딩 가이드 | 첫 진입 시 자동 표시, 종료 후 재호출 가능 | RQ-TRY-011 |
...
| 체험 모드 어뷰징·격리 | RQ-TRY-008·012·014·016 |
...
| 2026-04-30 | 오민성 (v3) | **§B/§C/§E 모두 1차 도입 확정**. (1) §16.2 미확정 결정 표에서 §B 자동충전 / §C 구독 / §E 후불 3행 모두 제거 — 잔존은 다중 IDC DR 1건뿐. (2) §16.3 결정 완료 항목에 자동충전·구독·후불 3건 추가, 영향 NFR(NFR-RELY-203·204·201, NFR-COMP-104·201, NFR-OPS-205) 명시. (3) **callback/destaddr 명명 통일** 반영 — RQ-CALLER → RQ-CALLBACK 일괄 치환, NFR-DATA-004 CSV 헤더 `to_number` → `destaddr`, NFR-AVAIL-006 `/try` → `/try`. (4) NFR-PERF-105 회원 콘솔 대시보드 경로 `/my` → `/dashboard` 갱신. NFR-* ID 변경 없음. |
